...
export function getColorScheme(stageIdx: number) {
  return {
    ...
    border: `border ${
      stageIdx == 0 ? "border-pink-600" : "border-emerald-600"
    }`,
    text: stageIdx == 0 ? "text-pink-600" : "text-emerald-600",
    textLight: stageIdx == 0 ? "text-pink-100" : "text-emerald-100",
    performerText: stageIdx == 0 ? "text-rose-200" : "text-emerald-200",
    performerBg:
      stageIdx == 0 ? "bg-rose-700" : "from-emerald-400 to-emerald-600",
  };
}
